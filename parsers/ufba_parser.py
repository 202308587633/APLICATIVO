...
class UFBAParser(BaseParser):
    ...
    def extract_pure_soup(self, html_content, url, on_progress=None):
        ...
        if on_progress: on_progress("UFBA: Analisando estrutura da página...")
        ...
        try:
            ...
            # <meta name="DC.publisher" content="Programa de Pós-graduação em Direito (PPGD) ...">
            ...
            if found_program:
                # Limpeza: 
                # 1. Remove "Programa de Pós-graduação em"
                clean_name = re.sub(
                ...
                # 2. Remove siglas entre parênteses no final, ex: "(PPGD)"
                clean_name = re.sub(r'\s*\([A-Z0-9-]+\)\s*$', '', clean_name)
                
                data['programa'] = clean_name.strip()
                if on_progress: on_progress(f"UFBA: Programa identificado: {data['programa']}")

        except Exception as e:
            if on_progress: on_progress(f"UFBA: Erro ao extrair programa: {str(e)[:20]}")

        # --- 2. EXTRAÇÃO DO PDF ---
        try:
            if on_progress: on_progress("UFBA: Buscando arquivo PDF...")
            ...
            if pdf_url:
                ...
                if on_progress: on_progress("UFBA: PDF localizado.")
            else:
                if on_progress: on_progress("UFBA: PDF não encontrado diretamente.")

        except Exception as e:
            if on_progress: on_progress(f"UFBA: Erro PDF: {str(e)[:20]}")
