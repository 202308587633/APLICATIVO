...
class ScraperController:
    ...
    def __init__(self, root):
        ...
        # --- CARREGAMENTO AUTOMÁTICO DA ABA 2 ---
        # Dispara a busca no banco assim que o app abre
        threading.Thread(target=self.load_programs_from_db, daemon=True).start()
        
        self.check_queue()

    def load_programs_from_db(self):
        """Busca os programas do banco e atualiza a view."""
        try:
            programs = self.db.get_all_programs()
            if programs:
                # Atualiza a UI na thread principal
                self.root.after(0, lambda: self.view.load_programs_data(programs))
                self.msg_queue.put(('status', f"Base de Programas: {len(programs)} registros carregados."))
            else:
                self.msg_queue.put(('status', "Base de Programas vazia. Execute o script de importação."))
        except Exception as e:
            self.msg_queue.put(('status', f"Erro ao carregar programas: {e}"))
    ...
    def _open_html_text(self, content, prefix):
        """Salva string HTML em arquivo temporário e abre no navegador."""
        try:
    ...
    def run_loop(self, data):
        ...
        try:
            while self.is_running:
                ...
                # Salva o HTML da página de busca
                if search_html:
                    self.db.save_search_page(data['engine'], data['term'], data['year'], page, search_html)

                if not items: 
                    self.msg_queue.put(('done', f"Fim da coleta. {msg} Total coletado: {total}"))
                    break
    ...
    def load_initial_data(self):
        try:
            ...
            if registros:
                self.view.update_status(f"Histórico carregado: {len(registros)} registros.")
        except Exception as e:
            self.view.update_status(f"Erro ao carregar histórico: {e}")

    def refresh_table_data(self):
        """Recarrega os dados do banco na tabela."""
        self.view.clear_table()
        self.load_initial_data()
        self.view.update_status("Tabela atualizada com dados do banco.")
    ...
    def open_stored_html(self, db_id, target_type):
        ...
        if html:
            self._open_html_text(html, f"{target_type}_{db_id}")
        else:
            self.view.update_status(f"HTML ({target_type}) não encontrado no banco.")
    ...
    def _redownload_search_page(self, termo, ano, pagina):
        """Baixa novamente o HTML da lista de busca e atualiza o banco."""
        def _task():
            try:
    ...
    def retry_info_fetch(self, db_id, link_repo):
        def _task():
            ...
            # Correção específica UFPR (exemplo legado)
    ...
    def handle_item_action(self, action, db_id, target_type, url=None):
        if action == 'view':
            ...
            if url and url != "-":
                ...
                self.view.update_status("URL inválida.")
    ...
    def set_manual_sigla(self, db_id):
        ...
        if nova_sigla is not None:
            ...
            try:
                ...
                self.view.update_status(f"Erro: {e}")
    ...
    def manage_field(self, db_id, field_type):
        try:
            if field_type == 'extracted_data':
                ...
                # Também limpa o link do repositório para forçar novo parse do zero
            ...
            self.view.update_status(f"Erro ao limpar: {e}")